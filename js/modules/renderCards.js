...
const createProductCard = (item, productCardContainer) => {
  let sizes = "";
  ...
  const card = `<div class="card" data-id="${item.id}">
                <div class="card-image">
                  <img
                    class="card-img"
                    src="goods-image/${item.preview}"
                    alt="${item.name}"
                  />
                  <div class="card-overlay">
                    <button class="card-overlay-btn add-to-cart-btn">
                      <svg
                        width="26"
                        height="23"
                        viewBox="0 0 33 29"
...
//функция отрисовки товаров на страница, если с id=feature - выводим на Главную, если нет - в каталог
const renderToPage = (productCardSection) => {
  if (productCardSection.id === "feature") {
    getData().then((data) => {
      data.forEach((item) => {
        if (item.feature) {
          createProductCard(item, productCardSection);
        }
      });
    });
  } else {
    getData().then((data) => {
      for (let i = 0; i <= 5; i++) {
        createProductCard(data[i], productCardSection);
      }
      // data.forEach((item) => {
      //   createProductCard(item, productCardSection);
      // });
